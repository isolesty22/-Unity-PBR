Shader "Unlit/BlinnPhongAddLight"
{
    Properties
    {
    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float3 NDotH = saturate(dot(wNormal,halfDir));
                float3 blinnPhong = pow(NDotH, _Smoothness * 100);
                //float4 specular = float4(phong,1) * _LightColor0 * float4(specularTint,1);
                float4 specular = float4(blinnPhong,1) * float4(light.color,1);
